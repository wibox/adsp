...
class Trainer():
    def __init__(self,
        ...
        self.layerwise_params = {"encoder*": dict(lr=self.encoder_learning_rate, weight_decay=self.layerwise_params_weight_decay)}
        self.model_params = utils.process_model_params(self.model, layerwise_params=self.layerwise_params)
        self.base_optimizer = RAdam(self.model_params, lr=self.learning_rate, weight_decay=self.adam_weight_decay)
        self.optimizer = Lookahead(self.base_optimizer)
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, factor=0.25, patience=2)
    ...
    def _train(self, loaders : Dict[str, DataLoader] = None) -> None:
        runner = SupervisedRunner(device=self.device, input_key="image", input_target_key="mask")
        ...
        runner.train(
            model=self.model,
            criterion=self.criterion, # da creare
